...
const createLink = async (req, res) => {
  try {
    const { fileId, permission, expirationTime, password } = req.body;
    ...
    else expDate.setDate(expDate.getDate() + 30); // Default to 30 or custom logic
    ...
    const newLink = await SharedLink.create({
      ...
      password // if hashed or raw, we'll store raw here for simplicity though bad practice, better to limit to prompt reqs
    });
